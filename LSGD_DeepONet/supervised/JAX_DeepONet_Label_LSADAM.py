...
if in_dims == 2:
    if in_ftn == 'kappa':
        u_in_foo = jnp.zeros((batch_size,Nx,Ny,1)) # 2D input
    else:
        u_in_foo = jnp.zeros((batch_size,Nx+1,Ny+1,1)) # 2D input
else:
    if in_ftn == 'flat_BC':
        u_in_foo = jnp.zeros((batch_size,4*Nx+1)) # 1D input
    elif in_ftn == 'Advec_PQ':
        u_in_foo = jnp.zeros((batch_size,2*Nx+1)) # 1D input
    else:
        u_in_foo = jnp.zeros((batch_size,Nx+1)) # 1D input
