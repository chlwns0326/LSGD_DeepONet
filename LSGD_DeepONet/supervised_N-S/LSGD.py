...
def solve_LS(params, u_in, u_out, xy, weights):
    ...
    _, dBB, VBBT = jnp.linalg.svd(B.T @ B, hermitian=True)
    _, dTT, VTTT = jnp.linalg.svd(T.T @ T, hermitian=True)
    
    E_tilde = VBBT @ E @ VTTT.T
    h_coeff = jnp.outer(dBB,dTT) + LSSlamb_regul*jnp.ones_like(jnp.outer(dBB,dTT))
