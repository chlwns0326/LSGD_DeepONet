...
# Directories (Current directory: /LSGD_DeepONet/supervised_N-S)
folder = 'Models_v3/'
keyword = 'NS_VS'
model_desc = 'L3W150'
datafolder, dataname = '../data/', 'NS_data'

# Hyperparameters
LS_num = 1 # Adam = 0, LS+Adam > 0
weights_init = [1,1e-6] # weights for data, regularization term, resp. 
N_train, N_val = 1000, 100 # Number of training/validation function dataset 
...
midt, tart = 10, 20 # data timestamp
...
if LS_num > 0: # LS+Adam
      ...
      model_dir = folder + keyword + '_0_' + str(midt) + 'to' + str(tart) + '_LSAdam_R' + str(adam_num) + \
          '_' + str(delay_init_pre) + '_HH_SS_bat'+ str(batch_size) + 'to' + str(batch_new) + '_' + \
      ...
      total_WU = 50000 // adam_num 
      delay_init = delay_init_pre
      model_dir = folder + keyword + '_0_' + str(midt) + 'to' + str(tart) + '_Adam_NoLS_HH_SS_bat' + \
            str(batch_size) + '_' + model_desc + '_seed_' + str(MasterKey)

createFolder(model_dir +'/models')
createFolder(model_dir +'/losses')

# Hyperparams for models
Nx, Ny = 64, 64 # Uniform grid of size (64)*(64)
xmin, xmax = 0.0,1.0 
ymin, ymax = 0.0,1.0
disp_count = int(0.002*total_WU) # tqdm progress display as new lines 

# Initialize
m = Nx*Ny  # number of trunk input sensors
u_in_foo = jnp.zeros((batch_size,Nx+1,Ny+1,midt+1)) # 2D input
...
j = 1 # jump/stride
uin_train = jnp.asarray(data['output_train'].astype('float64'))[:N_train,:,:,:midt+1][:,::j,:,:][:,:,::j,:]
uin_val = jnp.asarray(data['output_val'].astype('float64'))[:N_val,:,:,:midt+1][:,::j,:,:][:,:,::j,:]  
uout_train = jnp.asarray(data['output_train'].astype('float64'))[:N_train,:,:,tart][:,::j,:][:,:,::j]
uout_val = jnp.asarray(data['output_val'].astype('float64'))[:N_val,:,:,tart][:,::j,:][:,:,::j]
