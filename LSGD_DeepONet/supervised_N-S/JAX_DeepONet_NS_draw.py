...
Nx, Ny = 64, 64 # Uniform grid of size (32+1)*(32+1)
xmin, xmax = 0.0, 1.0 
ymin, ymax = 0.0, 1.0
Q_size = [Nx+1, Ny+1]
...
dataname = 'NS_data'
model_settings = [([[16,(3,3),(2,2),'VALID'],[32,(2,2),(2,2),'VALID'],[64,(2,2),(2,2),'VALID']],[150],silu,1,False),([150]*3,silu,1,False)]
branch_model = CNN_MLP(*model_settings[0])
trunk_model = MLP(*model_settings[1])
m = Nx*Ny  # number of trunk input sensors
u_in_foo = jnp.zeros((N_train,Nx+1,Ny+1,midt+1)) # 2D input
...
uin_val = jnp.asarray(data['output_val'].astype('float64'))[:N_test,:,:,:midt+1][:,::j,:,:][:,:,::j,:]  
uout_train = jnp.asarray(data['output_train'].astype('float64'))[:N_train,:,:,tart][:,::j,:][:,:,::j]
uout_val = jnp.asarray(data['output_val'].astype('float64'))[:N_test,:,:,tart][:,::j,:][:,:,::j]
...
for seed in seeds:
    for LS_num in LS_nums:
        ...
        if LS_num > 0: # LS+Adam
            ...
            model_dir = folder + keyword + '_0_' + str(midt) + 'to' + str(tart) + \
                '_LSAdam_R' + str(adam_num) + '_' + str(delay_init_pre) + '_HH_SS_bat'+ str(batch_size) + \
                'to' + str(batch_new) + '_' + model_desc + '_seed_' + seed + '_regul_' + reg_lambda 
        else: # Adam
            batch_size, batch_new = 200, 200 # predetermined adam batch 
            model_dir = folder + keyword + '_0_' + str(midt) + 'to' + str(tart) + \
                '_Adam_NoLS_HH_SS_bat'+ str(batch_size) + '_' + model_desc + '_seed_' + seed
