...
# Directories (Current directory: /LSGD_DeepONet/unsupervised/Poisson_g)
folder = 'Models_v3/'
keyword = 'Poisson_g_PI'
model_desc = 'L3W150'
datafolder = '../../data/'
dataname = 'Poisson_g_scale_data'

# Hyperparameters
LS_num = 1 # Adam = 0, LS+Adam > 0
weights_init = [1e-4,1,1e-8] # weights for physics, data, regularization term, resp. 
...
if LS_num > 0: # LS+Adam
    ...
    batch_new = 50
    adam_num = 2 # Adam epochs for each WU (LS step performed for each adam_num epochs) # 2 or 5
    total_WU = 160000 // adam_num # 200000
...
u_in_foo = jnp.zeros((batch_size,2*Nx+2*Ny+1)) # 1D input
